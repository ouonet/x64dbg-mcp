...
- [ ] T4: Bridge — classification + coalescing rules
...
- [ ] T5: Bridge — protocol v2 + `protocol.probe` + reject v1
...
- [ ] T6: Bridge — unsolicited event push channel
...
- [ ] T7: MCP — `BridgeClient` event dispatch + `protocol.probe` handshake
...
- [ ] T8: MCP — `SessionManager` state model + state-change CV
...
- [ ] T9: MCP — terminated session 30 s retention
...
- [ ] T10: MCP — remove 7 deprecated tools
...
- [ ] T11: MCP — execute_command description update
...
- [ ] T12: MCP — execution tools sync/async envelope
...
- [ ] T13: MCP — lifecycle tool returns + 60 s safety timeout
...
- [ ] T14: MCP — terminate / detach idempotency + cascade
...
- [ ] T15: MCP — read tools state snapshot
...
- [ ] T17: MCP — `save_memory_dump` + `create_minidump` tools
...
- [ ] T19: Update existing tests to new envelope shapes
...
- [ ] T20: New integration tests for state observability
...
- [ ] T21: Docs + .env.example + execute_command final review
